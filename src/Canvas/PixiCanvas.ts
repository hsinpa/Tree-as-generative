...
import { DrawCtrlPoint, DrawImage, DrawLine, DrawRect2D, Drawsphere } from "./PixiHelper";
import { vec2 } from "gl-matrix";
import WebglResource from "../Hsinpa/WebglResource";
import { ImageOption } from "./CanvasHelper";
import { Config, ImagesPath, Leaf_Textures } from "../SpaceColonization/SC_Static";
import * as H_Color from '../Hsinpa/Color';
import { Choice, Clamp } from "../Hsinpa/UtilityFunc";
...
class PixiCanvas {
    ...
    private m_leaf_textures: Texture[];
    ...
    public get Renderer() { return this.m_pixi_app.renderer; }
    ...
    constructor(pixi_application: Application) {
        ...
        this.m_branch_g_container.interactive=false;
    ...
    public async init_canvas(holder_dom_query: string) {
        ...
        await this.m_pixi_app.init({ width: this.m_width, height: this.m_height });

        this.m_leaf_texture = await Assets.load(Choice(Leaf_Textures));

        this.m_pixi_app.renderer.background.color = new Color([1,1,1]);
    ...
    private draw_branch(branch: SC_Branch) {
        ...
        if (clone_branch_parent != undefined && branch.is_vertices_set && clone_branch_parent.is_vertices_set) {
            let color_strength = 1 - (branch.position[1] /  this.m_height);
            let fill_color = [ (102 / 255) * color_strength, (51 / 255) * color_strength, 0];

            let branch_rect = DrawRect2D(branch.branch_vertices[0], branch.branch_vertices[1], 
                clone_branch_parent.branch_vertices[0], clone_branch_parent.branch_vertices[1], 
                fill_color, branch_graphic_source);
        ...
        if (branch.is_valid_endpoint) {
            ...
            let ctrl_point_graphic = DrawCtrlPoint(
                {x: branch.position[0], y: branch.position[1], source: endpoint_graphic_source
                }
            );
    ...
    public set_branches(branches: Map<string, SC_Branch>) {
        this.m_end_point_list.length = 0;

        branches.forEach((value, key) => {
            let clone_branch = SC_Branch.convert(value);
